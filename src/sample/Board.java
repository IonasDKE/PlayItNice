    ...
    private static Pane  makeGrid( int width, int hight){
        ...
        for(int h = 0; h<hight; h++) {
            for (int w = 0; w <= width; w++) {
                ...
                pane.getChildren().add(new Circle(w*squareSize+xTranslation, h*squareSize+yTranslation, DOT_SIZE, Color.BLACK));
                if(h==(hight-1)) { pane.getChildren().add(new Circle(w*squareSize+xTranslation, h*squareSize+squareSize+yTranslation, DOT_SIZE, Color.BLACK));}
            }
        }
