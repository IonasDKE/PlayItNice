...
public class RuleBasedNoDD extends AISolver {
    ...
    public int nextMove(State board, int color, String str) {
        ...
        Line result = null;
        //First phase: adding a 1st, or 2nd line is possible

        //First phase: check if any square can be claimed, if not pick a random line of valence 3 or 4
        ...
        //Second phase:
            ...
            System.out.println("RBNoDD entered phase 2");
            result = completeSquare();
            if (result == null) {
                result = colorEmptyLine();
            }
        }
        return result.getId();
    }

    //this method returns a random empty line, regardless of its valence.
    public static Line colorEmptyLine() {
        //System.out.println("2nd Phase: Colour any empty line");
        Random rand = new Random();
        ArrayList<Line> lines = GridController.getLines();
        ArrayList<Line> emptyLines = new ArrayList<Line>();

        //remove filled lines from list
        for (Line aLine : lines){
            if(aLine.isEmpty()){
                emptyLines.add(aLine);
            }
        }
        //select a random empty line
        int index = rand.nextInt(emptyLines.size());
        return emptyLines.get(index);
    }
